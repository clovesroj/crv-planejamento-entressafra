...
| Gasto real do ERP na Reforma (extração do Power BI) | `scripts/gasto-reforma-bi.mjs` — roda fora do servidor e grava `public/js/dados/gasto-reforma-bi.js`; o período pedido vira **fatias** (padrão mensal), soma deduplicando e grava a cada fatia, então `--inicio=2025-01-01 --fim=2026-09-23` vai numa chamada só, e rodar de novo retoma de onde parou |
| Janela do gasto real (o que conta como "já gasto") | `janelaGastoReal()` (`calculo/gasto-real.js`) — período, empresa, próprio e reforma do painel de filtros valem para a aba inteira: análise, grade de conjuntos, produtos e rastro. Não leia `GR_*` direto |
