...
import { APOIO, APOIO_FIXO, ARREND, ARR_PAR, ARR_RAT, ATVX, ATVX_V, BEN, CRM, CTT_NOVOS, CTT_SAIDAS, CTT_MUDANCAS, CTT_OBS, CTT_GERENCIAS_NOVAS, CTT_FUNCOES_NOVAS, CTT_CIDADES_NOVAS, CTT_CNH_NOVAS, CTT_DESLIG, CTT_DESLIG_META, DIESEL_MES, DIM, EDITADO, ENC, ESPOR, FAM_NOME, FAM_CLASSE, FAT, MO_APOIO, FORN, FORN_PAR, FROTA, CRM_ESP, MAQ, FROTA_UN, REAL, GRAT, GRUPOS_INS, INSUMO, INS_DEL, INSX, INSX_V, MATX, NIV, P, PLANO, QUADRO, ADM, ADM_RAT, TERC_TAR, TERC_SUB, TPESS, TRATC, TRAT_ATIVO, TRAT_DEL, TRAT_ETAPA, TRAT_NOME, TRAT_OBS } from '../nucleo/estado.js';
import { setAPOIO, setAPOIO_FIXO, setARREND, setARR_PAR, setARR_RAT, setATVX, setATVX_V, setBEN, setCRM, setCTT_NOVOS, setCTT_SAIDAS, setCTT_MUDANCAS, setCTT_OBS, setCTT_GERENCIAS_NOVAS, setCTT_FUNCOES_NOVAS, setCTT_CIDADES_NOVAS, setCTT_CNH_NOVAS, setCTT_DESLIG, setCTT_DESLIG_META,
         setDIESEL_MES, setDIM, setEDITADO, setFAM_NOME, setFAM_CLASSE, setFORN, setFORN_PAR, setENC, setESPOR, setFAT, setMO_APOIO, setFROTA, setCRM_ESP, setMAQ, setFROTA_UN, setREAL, setGRAT, setGRUPOS_INS, setINSUMO, setINS_DEL,
...
function estadoCru(){
  return {P,PLANO,DIM,INSUMO,INS_DEL,ESPOR,TRATC,TRAT_NOME,TRAT_OBS,TRAT_ETAPA,TRAT_DEL,TRAT_ATIVO,DIESEL_MES,ARREND,ARR_PAR,ARR_RAT,FORN,FORN_PAR,ENC,BEN,NIV,GRAT,APOIO,APOIO_FIXO,
          TERC_TAR,TERC_SUB,CRM,CRM_ESP,MAQ,FROTA_UN,REAL,MATX,INSX,INSX_V,ATVX,ATVX_V,CTT_NOVOS,CTT_SAIDAS,CTT_MUDANCAS,CTT_OBS,CTT_GERENCIAS_NOVAS,CTT_FUNCOES_NOVAS,CTT_CIDADES_NOVAS,CTT_CNH_NOVAS,CTT_DESLIG,CTT_DESLIG_META,FROTA,TPESS,FAT,MO_APOIO,QUADRO,ADM,ADM_RAT,GRUPOS_INS,FAM_NOME,FAM_CLASSE,FUN:CFG.funcoes.map(f=>f.sal),v:10};
}
...
const CHAVES_COM_PATCH = { INSX_PATCH: ["INSX"], ATVX_PATCH: ["ATVX"], INSUMO_PATCH: ["INSUMO"],
  ...
  CTT_NOVOS_PATCH: ["CTT_NOVOS"], CTT_SAIDAS_PATCH: ["CTT_SAIDAS"], CTT_MUDANCAS_PATCH: ["CTT_MUDANCAS"], CTT_OBS_PATCH: ["CTT_OBS"] };
...
function aplicar(d){
  ...
  if(d.INS_DEL) setINS_DEL(d.INS_DEL);
  ...
  if(d.CTT_MUDANCAS) setCTT_MUDANCAS(d.CTT_MUDANCAS);
  if(d.CTT_OBS) setCTT_OBS(d.CTT_OBS);
  if(d.CTT_GERENCIAS_NOVAS) setCTT_GERENCIAS_NOVAS(d.CTT_GERENCIAS_NOVAS);
  if(d.CTT_FUNCOES_NOVAS) setCTT_FUNCOES_NOVAS(d.CTT_FUNCOES_NOVAS);
  if(d.CTT_CIDADES_NOVAS) setCTT_CIDADES_NOVAS(d.CTT_CIDADES_NOVAS);
  if(d.CTT_CNH_NOVAS) setCTT_CNH_NOVAS(d.CTT_CNH_NOVAS);
  if(d.CTT_DESLIG) setCTT_DESLIG(d.CTT_DESLIG);
  if(d.CTT_DESLIG_META) setCTT_DESLIG_META(d.CTT_DESLIG_META);
  ...
  // so quando vem: aplicar() tambem recebe o documento parcial de uma
  // alteracao pendente (recuperarPendente), e zerar aqui apagaria o que o
  // servidor tem
  if(d.FAT) setFAT(d.FAT);
  if(d.MO_APOIO) setMO_APOIO(d.MO_APOIO);
