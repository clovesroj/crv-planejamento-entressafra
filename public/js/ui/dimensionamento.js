...
function pintarDim(R){
  ...
  // atividade que vai junto de outra (PL05 e PL06 na plantadora da PL04) nao tem
  ...
  $("#t_dim").innerHTML = th([["Cod"],["Atividade / frente"],["Etapa"],
    ...
    L.map(r=>{
      ...
      return `<tr><td>${esc(r.a.cod)}</td><td>${r.a.nome}${lj.length?` <span class="calc" title="Na mesma passada: a frota e a equipe desta linha fazem também ${lj.join(" e ")}">+ ${lj.join(", ")}</span>`:""}</td>
...
function pintarDimDetalhe(R){
  ...
  cont.innerHTML = `
    ...
    <div class="ra-topo">
      ...
      <div class="ra-tit">${esc((r.a.cod))} · ${esc(r.a.nome)}</div>
