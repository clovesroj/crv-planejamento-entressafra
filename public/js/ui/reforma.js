          ...
          m.linhas.map(l=>
            `<tr><td style="padding-left:22px">${l.cod}</td>
             ...
             ${cols.map(c=>{ const real = gastoRealDe(e.familia, l.cod, c);
               ...
               return `<td class="num"><input data-ref="${l.cod}" data-c="${c}" value="${
...
/** Gasto real (ERP/Power BI) de uma unidade num conjunto, somando as tags mapeadas. Null sem dado. */
function gastoRealDe(familia, cod, conjunto){
  const porFrota = GASTO_REFORMA_BI.porFrota[cod];
  if(!porFrota) return null;
  const tags = tagsBiDoConjunto(familia, conjunto);
  const soma = tags.reduce((s,tag)=> s + (porFrota[tag]?.total || 0), 0);
  return soma || null;
}
